...
##### 7_11 凌晨TAAC1训练中 7_11_kfold_v1/
def model_7_11_v1(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_11_v1 : embedding 300 + share_tran +  num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
#### embedding 300 concat + num_heads=6 + trainable adv pid
def model_7_11_v2(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_11_v2 : embedding 300 + concat +  num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
    ...
    embed = concatenate([x_embed, y_embed,z_embed])

    embed_transformer = transformer_block(embed)

    tf_idf_embedding = Activation(activation="relu")(BatchNormalization()(Dense(200)(tf_idf)))
    countvec_embedding = Activation(activation="relu")(BatchNormalization()(Dense(20)(countvec)))


    x = Dropout(0.3)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(embed_transformer))
    semantic = TimeDistributed(Dense(100, activation="relu"))(x)
    pool_rnn = Lambda(lambda x: K.max(x, axis=1), output_shape=(100,))(semantic)
    semantic_attn = AttentionWeightedAverage()(semantic)
...
####先试一试v1 v2哪个好 ==>trainable or not trainable
#### 下面对应改

##### model_7_11_v1 改版  去掉semantic
def model_7_11_v3(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_11_v3 : embedding 300 + share_tran +  num_heads=6 + trainable adv pid")
    dropout_param = 0.3
    ...
    embed_trans = concatenate([x_embed_transformer, y_embed_transformer,z_embed_transformer])

    embed_lstm = Dropout(dropout_param)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(embed_trans))

    embed_att = AttentionLayer(attention_size=190)(embed_lstm)

    features = concatenate([embed_att,max_pool(embed_lstm),max_pool(embed_trans),max_pool(lstm1),tf_idf_embedding,countvec_embedding])  
