...
def tran_lstm_model_v1(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("tran_lstm_model_v1")
    ...
    x_embed_transformer = transformer_block(x_embed)
    y_embed_transformer = transformer_block(y_embed)
    z_embed_transformer = transformer_block(z_embed)
    ...
    x = concatenate([x_embed_transformer, y_embed_transformer,z_embed_transformer])

    x = Dropout(0.1)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(x))
    semantic = TimeDistributed(Dense(100, activation="relu"))(x)
    pool_rnn = Lambda(lambda x: K.max(x, axis=1), output_shape=(100,))(semantic)
    semantic_attn = AttentionWeightedAverage()(semantic)

    features = concatenate([pool_rnn, semantic_attn,tf_idf_embedding,countvec_embedding])


    x = Dropout(0.1)(Activation(activation="relu")(BatchNormalization()(Dense(1000)(features))))
    x_age = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    x_gender = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    output_age = Dense(10, activation="softmax",name='output_age')(x_age)
    output_gender= Dense(2, activation="softmax",name='output_gener')(x_gender)




    # model = Model(inputs=[x_ids,y_ids,z_ids,a_ids], outputs=[output_age,output_gender])
    model = Model(inputs=[x_ids,y_ids,z_ids,tf_idf,countvec], outputs=[output_age,output_gender])
    model.compile(optimizer=RAdam(lr=0.001,weight_decay=0.08,), loss='categorical_crossentropy',
...
def tran_lstm_model_v1_v3_muti(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("tran_lstm_model_v1_v3_muti")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def capsule_v1(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("capsule_v1")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    ...
    tf_idf_embedding = Dropout(0.1)((Dense(200)(tf_idf)))
    tf_idf_embedding = Activation(activation="relu")(BatchNormalization()(Dense(100)(tf_idf_embedding)))

    countvec_embedding = Dropout(0.1)((Dense(20)(countvec)))
    countvec_embedding = Activation(activation="relu")(BatchNormalization()(Dense(20)(countvec_embedding)))
    ...
    x = concatenate([x_embed,x_embed_transformer, y_embed,y_embed_transformer,z_embed,z_embed_transformer])
    ...
    # semantic = TimeDistributed(Dense(100, activation="relu"))(x)
    # pool_rnn = Lambda(lambda x: K.max(x, axis=1), output_shape=(100,))(semantic)
    # attn = AttentionWeightedAverage()(semantic)

    # features = concatenate([capsule,pool_rnn,attn,tf_idf_embedding,countvec_embedding])
    features = concatenate([capsule,tf_idf_embedding,countvec_embedding])
...
def transformer_7_2_v1(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("transformer_7_2_v1")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
    ...
    x = Dropout(0.2)(Bidirectional(CuDNNLSTM(400, return_sequences=True))(x))
    semantic = TimeDistributed(Dense(400, activation="relu"))(x)
    pool_rnn = Lambda(lambda x: K.max(x, axis=1), output_shape=(400,))(semantic)
    semantic_attn = AttentionWeightedAverage()(semantic)

    features = concatenate([pool_rnn, semantic_attn,tf_idf_embedding,countvec_embedding])


    x = Activation(activation="relu")(BatchNormalization()(Dense(1000)(features)))
    x_age = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    x_gender = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    output_age = Dense(10, activation="softmax",name='output_age')(x_age)
    output_gender= Dense(2, activation="softmax",name='output_gener')(x_gender)
...
def model_7_3_v1_muti(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_3_v1_muti : mdoel v1 + embedding 300 + num_heads=6")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def model_7_3_v2_muti(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_3_v1_muti : mdoel v1 + embedding 300 + num_heads=12")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def tran_lstm_model_v1_addcid_muti(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,a_embeddings_weight,class_num):
    print("tran_lstm_model_v1_addcid_muti")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    a_ids = Input(shape=(sent_length,), dtype='int32')
    ...
    y_embed = y_embedding(y_ids)
    z_embed = z_embedding(z_ids)
    a_embed = z_embedding(a_ids)

    x_embed_transformer = transformer_block(x_embed)
    y_embed_transformer = transformer_block(y_embed)
    z_embed_transformer = transformer_block(z_embed)
    a_embed_transformer = transformer_block(a_embed)


    tf_idf_embedding = Activation(activation="relu")(BatchNormalization()(Dense(200)(tf_idf)))
    countvec_embedding = Activation(activation="relu")(BatchNormalization()(Dense(20)(countvec)))

    x = concatenate([x_embed_transformer, y_embed_transformer,z_embed_transformer,a_embed_transformer])
    ...
    # model = Model(inputs=[x_ids,y_ids,z_ids,a_ids], outputs=[output_age,output_gender])
    model = Model(inputs=[x_ids,y_ids,z_ids,a_ids,tf_idf,countvec], outputs=[output_age,output_gender])

    # lookahead = Lookahead(k=5, alpha=0.5) # Initialize Lookahead
    # lookahead.inject(model) # add into model
    return model

def transformer_7_2_v1_trainable(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("transformer_7_2_v1_trainable")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def model_7_3_v1__trainable(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_3_v1__trainable : mdoel v1 + embedding 300 + num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def diff_dim_concat_kfold_v2(sent_length, x_embeddings_weight,class_num):
    print("diff_dim_concat_kfold_v2 : mdoel v1 + embedding 200 + num_heads=8 + trainable aid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
    countvec = Input(shape=(12,), dtype='float32')
    ...
    x = Dropout(0.1)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(x_embed_transformer))
    semantic = TimeDistributed(Dense(100, activation="relu"))(x)
    pool_rnn = Lambda(lambda x: K.max(x, axis=1), output_shape=(100,))(semantic)
    semantic_attn = AttentionWeightedAverage()(semantic)
    ...
    model = Model(inputs=[x_ids,tf_idf,countvec], outputs=[output_age,output_gender])
    return model

def diff_dim_concat_kfold_v2_duibi(sent_length, x_embeddings_weight,class_num):
    print("diff_dim_concat_kfold_v2 : mdoel v1 + embedding 200 + num_heads=8 + trainable aid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
    countvec = Input(shape=(12,), dtype='float32')
...
def model_tran_dimdiff_concat_trainable(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,a_embeddings_weight,b_embeddings_weight,class_num):
    print("model_tran_dimdiff_concat_trainable : tran + diff dim +concat + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    a_ids = Input(shape=(sent_length,), dtype='int32')
    ...
    embed = concatenate([x_embed, y_embed,z_embed,a_embed,b_embed])
    print('########################x_embed',embed.get_shape())
    ...
    x = transformer_block(embed)    
#     x = Dropout(0.1)(Bidirectional(CuDNNLSTM(200, return_sequences=True))(embed))
    x = Dropout(0.1)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(x))
...
def model_lstm_dimdiff_concat_trainable_plus(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,a_embeddings_weight,b_embeddings_weight,class_num):
    print("model_lstm_dimdiff_concat_trainable_plus : lstm +diff dim +concat + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    a_ids = Input(shape=(sent_length,), dtype='int32')
    ...
    x = Dropout(0.1)(Bidirectional(CuDNNLSTM(400, return_sequences=True))(embed))
    x = Dropout(0.1)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(x))
...
def model_7_3_v1__trainable(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    ...
    x = Dropout(0.2)(Activation(activation="relu")(BatchNormalization()(Dense(1000)(features))))
    x_age = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    x_gender = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    output_age = Dense(10, activation="softmax",name='output_age')(x_age)
    output_gender= Dense(2, activation="softmax",name='output_gener')(x_gender)
...
def model_wgd(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,a_embeddings_weight,b_embeddings_weight,class_num):
    print("model_wgd : lstm +diff dim +concat + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    a_ids = Input(shape=(sent_length,), dtype='int32')
...
#     tf_idf_embedding = Activation(activation="relu")(BatchNormalization()(Dense(200)(tf_idf)))
    ...
    lstm_layer = Bidirectional(CuDNNLSTM(190, return_sequences=True, kernel_initializer=initializers.glorot_uniform(seed = 123)))
...
def model_wgd_22(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,a_embeddings_weight,b_embeddings_weight,class_num):
    print("model_wgd_22 : lstm +diff dim +concat + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    a_ids = Input(shape=(sent_length,), dtype='int32')
    ...
    x = Dropout(dropout_param)(Activation(activation="relu")(BatchNormalization()(Dense(1200)(features))))
    x = Dropout(dropout_param)(Activation(activation="relu")(BatchNormalization()(Dense(800)(x))))
    x = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))

    pred = Dense(20, activation='softmax')(x)
    # 修改
    # model = Model(inputs=[x_ids,y_ids,z_ids,a_ids,b_ids,tf_idf,countvec], outputs=pred)
    model = Model(inputs=[x_ids,y_ids,z_ids,a_ids,b_ids,countvec], outputs=pred)
    return model




def model_v1(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_v1 : mdoel v1 + embedding 300 + num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def model_7_3_v1__trainable_v2_bk(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_3_v1__trainable_v2 : mdoel v1 + embedding 300 + num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def model_7_3_v1__trainable_v2(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_3_v1__trainable_v2 : mdoel v1 + embedding 300 + num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    ...
    x = Dropout(0.3)(Bidirectional(CuDNNLSTM(100, return_sequences=True))(x))
    semantic = TimeDistributed(Dense(100, activation="relu"))(x)
    pool_rnn = Lambda(lambda x: K.max(x, axis=1), output_shape=(100,))(semantic)
    semantic_attn = AttentionWeightedAverage()(semantic)
    ...
    x = Dropout(0.3)(Activation(activation="relu")(BatchNormalization()(Dense(1000)(features))))
    x_age = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    x_gender = Activation(activation="relu")(BatchNormalization()(Dense(500)(x)))
    output_age = Dense(10, activation="softmax",name='output_age')(x_age)
    output_gender= Dense(2, activation="softmax",name='output_gener')(x_gender)
...
def model_7_3_v1__trainable_v3(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,class_num):
    print("model_7_3_v1__trainable_v3 : mdoel v1 + embedding 300 + num_heads=6 + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    tf_idf = Input(shape=(sent_length,), dtype='float32')
...
def model_7_11_wgd_v1(sent_length, x_embeddings_weight,y_embeddings_weight,z_embeddings_weight,a_embeddings_weight,b_embeddings_weight,class_num):
    print("model_7_11_wgd_v1 : lstm +diff dim +concat + trainable adv pid")
    x_ids = Input(shape=(sent_length,), dtype='int32')
    y_ids = Input(shape=(sent_length,), dtype='int32')
    z_ids = Input(shape=(sent_length,), dtype='int32')
    a_ids = Input(shape=(sent_length,), dtype='int32')
